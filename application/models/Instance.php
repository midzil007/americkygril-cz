...
//		$_COOKIE = null;
		...
		if($view->inputGet->souv){
			$mImportWeb = new module_ImportWeb();
			pr($mImportWeb->setSouvisejici());
			die;  
		}

		if($view->inputGet->readC){
			e("Ds");   
			$mImportWeb = new module_ImportWeb();
		//	pr($mImportWeb->setSklademPropertyCsv());  
			die;  
		}
		...
		if(!isset($_SESSION['historyBasketCount1'])){
			$_SESSION['historyBasketCount1'] = $view->historyBasketCount = Utils::loadUrl('https://www.nemeckekavovary.cz/?basketHistory=1','2');
			 } 
		else{
	...
	//$view->historyBasketCount = 107807;        
		...
		$view->newsUrl = '/weber/akcni-nabidky-slevy-a-novinky-do-vaseho-e-mailu';
		...
		$view->profileUrl = '/weber/vas-profil';
		$view->predkosik = '/weber/?predkosik=1';   
		$view->bp = '/weber/nakupni-kosik';
		$view->profileLogout = '/weber/vas-profil?logout=1';
		$view->basketUrl = '/weber/nakupni-kosik';
		$view->confirmUrl = '/weber/potvrzeni';
		...
		if($view->inputGet->weber){ 
				   
			$mAdmin = new module_ImportAdmin($view);      
			pr($mAdmin->setProducts("http://admin.specshop.cz/?renderWeber=1")); 
			die;   
		}
		if($view->inputGet->setDkdsafs){
			$mAdmin = new module_ImportWeb($view);
			$mAdmin->addActicle();          
			die;  

			$mAdmin = new module_Products();   
			$params['category'] = 78987; //     
		...
		if($view->inputGet->loadProducts){
			...
			{       
				...
				if(!is_numeric(strpos($it,"weber"))){  		
					$title = "Weber ".$it;      
					pr($title);     
					$nodeToUpdate = $view->tree->getNodeById($item['id']);
					$this->input = $nodeToUpdate;
					$this->input->title = $title;              
					$this->input->pageTitle = $title;     
					$nodeToUpdate->initUpdate($this->input, $this->view );          
					$view->tree->updateNode($nodeToUpdate, false);     
				}            
			}
		...
		if($view->inputGet->showProp){
			$mWeb = new module_ImportWeb();  
			pr($mWeb->setProducts(false));  
		}  
		...
		if($view->inputGet->offerSet == 1){  
			$mImportAdmin = new module_ImportAdmin($view);
			$mImportAdmin->setOffer($view,$view->inputGet->token);
			helper_FrontEnd::redirect(Utils::getWebUrl().$view->bp, 302); 
			die;            
		}
		...
		if($_GET['repath'])
		{
			...
			$path = str_replace("/jura","",$node->path);
		      
			$node->path = '/weber'.$path;    
			$node->cz_path = '/weber'.$path;          
			pr($node->path );             ;             
