...
$url="https://media.jura.com/360/s8/images/";    
...
for ($i=1; $i < 200; $i++) {         
	...
	pr($path);                       
		...
		if($content != false){                        
			e('/images/3d/'.$i.'.jpg');                                                  
			file_put_contents('images/3d/image_1_'.$i.'.jpg', $content);
		}   
		else{           
			break;       
		}              
...
die;         
		...
		if($_GET['getproducts']) 
		{
		    ...
		    if($_GET['nodeid']>0)
		    {
		    	//$params['onlykavovars'] = $_GET['onlykavovars'];    
		    	$mApi = new module_Api(); 
		    	$params['nodeid'] = $_GET['nodeid'];      
				$params['prislusenstvi'] = $_GET['prislusenstvi'];
		    	$orders['products'] = $mApi->getPrislusenstvi($_GET['nodeid']);  
		    }
			else{    
				$mApi = new module_Api();
...
echo json_encode($images);
		...
		if($_GET['importNiggaBro'] == 7){
			//http://show.eportaly.cz/?importNiggaBro=7&vyprodej=1&colors=0&start=0&end=5
			//$import = new module_ImportHeureka($view); 
		//	$import->import($_GET['category']); 	   
