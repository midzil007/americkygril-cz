...
class content_Product extends Content {
    ...
    public function __construct($config = array())
    {
		...
		$this->properties[] = new ContentProperty('text_dole','Wysiwyg','', array(),array(), array(  'height' => 250)); 
		$this->properties[] = new ContentProperty('photo_dole','MultiFileSelect',array('accordionId' => 'accordion', 'noContent' => true, 'showSelectFile' => true, 'inputWidth' => '300', 'maxFiles' => 60, 'showUploadFile' => true, 'uploadFileDirectoryNodeId' => 3242 ));
		$this->properties[] = new ContentProperty('text_ikon','Text','', array(),array(), array(  'height' => 250));  
		$this->properties[] = new ContentProperty('disable_product','Checkbox','', array(), array(), array(), false);	
		$this->properties[] = new ContentProperty('heureka-title','Hidden','', array(), array(), array(), false);     
    	$this->properties[] = new ContentProperty('parent','MultiPageSelect','', array(), array('root' => 3801, 'display' => 'FOLDER')); 
		$this->properties[] = new ContentProperty('zboziProduct','Hidden','', array(),array(), array(  'height' => 250));    		 		            	   
		...
		$this->properties[] = new ContentProperty('other_souvisejici','MultiPageSelect','', array(), array('root' => 3801, 'display' => 'ITEM', 'sort'=>'title'));
		$this->properties[] = new ContentProperty('prop','MultiPageSelect','', array(), array('root' => 76947, 'display' => 'ITEM', 'sort'=>'title'));
		
		$this->properties[] = new ContentProperty('method_gril','MultiPageSelect','', array(), array('root' => 79295, 'display' => 'ITEM', 'sort'=>'title'));
		$this->properties[] = new ContentProperty('garancy_product','MultiPageSelect','', array(), array('root' => 79294, 'display' => 'ITEM', 'sort'=>'title'));
			...
			if($item > 0){ 
				...
				if($node ){
					...
					$text .= '<div class="prop"><p><img src='.$authorPhoto['path'].' alt='.$node->title.'><p>
							<p>'.ucfirst($node->title).'</p></div>'; 
						}
					}
		}   
		return $text;
	}
	function renderGarancy($view,$garancy)
	{
		if($garancy)
		{
			$node = $view->tree->getNodeById($garancy);
			$article = $node->getPublishedContent();		  ;   
			$authorPhoto = helper_FrontEnd::getPhoto('photos', $article, $article->fotoShowName , $article->fotoShowName, 1);
			$object = new stdClass();
			$object->title = $node->title;
			$object->html = $article->getPropertyValue('html');
			$object->image = $authorPhoto ;
			return  $object; 
		}
	}
    ...
    function show($view, $node){
		...
		$view->text_dole= $this->getPropertyValue('text_dole');    
		$view->ikonText = $this->getPropertyValue('text_ikon');    
		$view->method_gril = $this->renderGarancy($view,$this->getPropertyValue('method_gril'));      
		$view->garancy = $this->renderGarancy($view,$this->getPropertyValue('garancy_product'));
		...
		$textPhoto = $view->content->getFilesNames('photo_dole'); 
		if($textPhoto)
			$view->textPhoto = helper_FrontEnd::getFirstPhoto($textPhoto,false,false);
		...
		$view->souvisejici = $this->getSouvisejici($view,'souvisejici'); 
		$other = $this->getSouvisejici($view,'other_souvisejici'); 
		if($other) 
			$view->souvisejici = array_merge($view->souvisejici,$other);
	...
	private function getSouvisejici($view,$name)
	{  
		$souvisejici = $this->getPropertyValue($name);      
		if($souvisejici) 
		{  
			$souv = array();  
