...
class content_Basket extends content_HtmlFile {
    ...
    function show($view, $node){    	    
		...
		if($view->input->kod)
		{
			...
			{
				helper_FrontEnd::redirect('/weber/nakupni-kosik?step=1&kod=' .$view->input->kod, 302, true);
    			die();  
			}    
		}
    	...
    	switch ($action){ 
    		...
    		case 'refreshPayment':  			
    			if($view->inputGet->ajax){
    				...
    				if($view->input->platby){
    					$view->mEshop->basket->setPayment($view->input->platby);
    				}
				...
				//if(!$view->showPrice) 
    				...
    				if(count($view->input->count)){	
	    				foreach ($view->input->count as $pid => $count){
	    					...
	    					if($count > 0){
	    						if($view->showPrice && $_SESSION['CMS7']['logged'] == 1)
	    						{ 
	    							$price = $items[$pid]['price'] ; 
	    							$view->mEshop->addItemByIdQuick($pid, $count,false,$price);	    						    
	    						}
								else{
		...
		if($view->step == 2 && $view->inputGet->step!=2){ 
			...
			//$delivery = $view->mEshop->basket->getDelivery();
			//$payment = $view->mEshop->basket->getPayment();
			if($view->input->dop && $view->input->platby){
				$view->mEshop->basket->setDelivery($view->input->dop);
    			$view->mEshop->basket->setPayment($view->input->platby); 
...
//			}  
    	...
    	if($view->step == 4){
			...
			if(!$stop || $view->inputGet->successPay) {        
				
				$sum = $view->mEshop->basket->getSum($view);
				
				list($fid,$qr) = $view->mEshop->basket->makeOrder($view);  	
				$payment = $view->mEshop->basket->getPayment();
				...
				if(!$fid){      
					helper_FrontEnd::redirect($view->bp . '?done=1&z=' . base64_encode(123456789).'&price='. base64_encode($sum) . '&p=' . $payment . '' . '&d=' . $delivery. '&news=1'.$add. '&qr='.$qr, 302);  
				} else{ 
					helper_FrontEnd::redirect($view->bp . '?done=1&z=' . base64_encode($fid).'&price='. base64_encode($sum). '&p=' . $payment . '' . '&d=' . $delivery. '&news=1'.$add. '&qr='.$qr, 302);  
				}
			}   
			$view->session->step = 0;   
		}
		...
		if($redir && !$disableRedir){  
			helper_FrontEnd::redirect('/weber/nakupni-kosik?step=' . $redir, 302, true);
		}
	}  
