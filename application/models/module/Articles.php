...
{   
	...
	public function initSearch( $sort = 'title', $sortType = 'Asc', $limitStart = 0, $limitCount = 5, $root, $count = false)    
    {
		...
		if($count){    		
    		...
    		$select->from(array( 'cm' => $this->_tableName), array('n.id', 'n.title', 'n.path', 'n.parent', 'dateCreate', 'html', 'dateShow', 'files')); 		  
    	}
    ...
    function  getArticlesAsNodes($tree, $articles,$sort = false){  
    	$nodes = array();
    	foreach ($articles as $article){ 
			$node = $tree->getNodeById($article['id']);
			$nodes[$node->orderValuation] = $node;
		}
		if($sort){
			ksort($nodes);
		}
