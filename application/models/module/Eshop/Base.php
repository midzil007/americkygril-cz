...
class module_Eshop_Base {
	...
	function itemSold($order){
		...
		if(count($details) > 0){
			foreach ($details as $detail){
				...
				$v = $mVariant->getVariantById($detail['variant'], false,false);
	//	e($v);
				$price = $detail['price'];    
	...
	function addItemById($id, $count,$view = false){  
		if($count > 0){ 
			$itemId= explode('_', $id);
	...
	function addItemByIdQuick($id, $count,$view,$price){  
		if($count > 0){ 
			$itemId= explode('_', $id);
			$product = $this->tree->getNodeById($itemId[0]);
			$mProd = new module_Products();
			$this->basket->addItemQuickShop($product, $count,$price,$view,$itemId[1],$itemId[2]);
		} else { 
			//$this->basket->removeItem($id); 
		}
