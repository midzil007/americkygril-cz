...
class module_Eshop_Basket {
	...
	function saveLastVisitedToCookie($nodeId, $variant) {
		...
		if ($inst) {
			...
			$this->setcookielive("products", $saveCookie, time()+36000000, '/', 'Americkygril.cz');
		}
	}
	...
	function addItemQuickShop($item, $count = 1, $price, $view, $variantId, $optionId, $voucher = false) {
		...
		$this->session->offerToken   = $view->offerToken;
	...
	function saveCookies($basket = false, $delivery = false, $payment = false, $note = false, $clear = false) {
		if ($_COOKIE['eu-cookies']) {
			if ($clear) {       
				$this->setcookielive("basket", null, time()+36000000, '/', 'Americkygril.cz');
				$this->setcookielive('delivery', null, time()+36000000, '/', 'Americkygril.cz');
				$this->setcookielive('note', null, time()+36000000, '/', 'Americkygril.cz');
				$this->setcookielive('payment', null, time()+36000000, '/', 'Americkygril.cz');
			}
			if ($basket) {
				$this->setcookielive("basket", '', time()+36000000, '/', 'Americkygril.cz');
				foreach ($basket as $key => $value) {  
					$cookie[$key] = $value['count'];
				};
				$saveCookie = base64_encode(serialize($cookie));
				$this->setcookielive("basket", $saveCookie, time()+36000000, '/', 'Americkygril.cz');
			}
			if ($delivery) {
				$this->setcookielive('delivery', $delivery, time()+36000000, '/', 'Americkygril.cz');
			}
			if ($payment) {
				$this->setcookielive('payment', $payment, time()+36000000, '/', 'Americkygril.cz');
			}
			if ($note) {
				$note = base64_encode($note);
				$this->setcookielive('note', $note, time()+36000000, '/', 'Americkygril.cz');
			}
		}
	}
	...
	function getSum($view = false, $deliveryPay = false) {
	...
	function makeOrder($view) {
		...
		if ($this->getItems) {
			...
			if($this->session->offerToken){
				$mIportAdmin->deactiveOffer($this->session->offerToken);
				unset($this->session->offerToken);
			}
			$items       = $this->getItems();
...
<p style="margin: 0; margin-bottom: 5px; margin-left: 5px;">'.$shopperBasket['ftelefon'].'</p>';
			...
			foreach ($this->getItems as $key => $odata) {
				...
				$_SESSION['idsFaceBook'][] = "weber_cz_".$child->nodeId;
			...
			$u = 'http://www.heureka.cz/direct/dotaznik/objednavka.php?id=c6b372319ffbcd9506ed7dad1d876770&email='.$data['email'].$zbozi.'&orderid='.$oid;
			file($u);
...
// 				    
				...
				$mail2->addTo('objednavky.specshop@gmail.com', 'Americkygril.cz');
				$mail2->addBcc('nosil@specshop.cz', 'Americkygril.cz');   
				$mail2->addBcc('prodej@specshop.cz', 'Americkygril.cz');
				$mail2->addBcc('obchod@specshop.cz', 'Americkygril.cz');
				$mail2->addBcc('info@specshop.cz', 'Americkygril.cz');  
				if($shopperBasket['payment'] == 6)
						$mail2->addBcc('fakturace@specshop.cz', 'Americkygril.cz');
    
				$mail2->setFrom('noreply@specshop.cz', 'Americkygril.cz');
