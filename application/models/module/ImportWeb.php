...
class module_ImportWeb {  
  ...
  //         '78401|78667' => 'https://www.weber.com/CZ/cs/p%C5%99%C3%ADslu%C5%A1enstv%C3%AD/grilov%C3%A1n%C3%AD/grilovac%C3%AD-n%C3%A1%C4%8Din%C3%AD/',
    ...
    public function setSouvisejici()
    {
        $mProducts = new module_Products();
        //$parama = grily Node;
        $params['category']         = 76931;
        $products = $mProducts->getProducts(false,false,0,999,$params);
      ...
      public function separete($url = false)
      {      
    ...
    public function setSklademPropertyCsv($start = 0, $end = 1)
	{
        // není madlo
        //$d['skladem'] = 2;
        //$datC['state'] = 'ARCHIVED';
        //$this->db->update("content_Product",$datC);
        //$this->db->update("module_eshop_variants",$d);    
        $file = $this->config->dataRoot . '/data4.csv';
		...
		if (($handle = fopen($file, "r")) !== FALSE) {
          ...
          foreach($prod as $key=>$item)
          {
              ...
              $dataItem = strlen($item)== 11? '00'.$item : $item;
  
              if($item){  
             // $where = $this->db->quoteInto("model =?",$item);
             // $dataVariant['skladem'] = 1;
           //   $this->db->update("module_eshop_variants",$dataVariant,$where);
              $id = $this->db->fetchOne("select id_product from module_eshop_variants where model =?",$key);
              $data['EAN'] = $dataItem;
              $where = $this->db->quoteInto("model = ?",$key);
              //pr($data);
              //pr($where);  
    ...
    {
        //dodělat aby nebyli seo SHow nebo jak se to jmenuje.
        $data = $this->db->fetchAll('select * from '. $this->_tableSepareteProd." where insert_product = '0' limit 20");

        foreach($data as $item)  
        {
            // nastavit aby se uložilo k produktu... a do separeteProduct nastavit, že se to již provedlo.<
            ...
            {     
                ...
                {      
                   ...
                   // pr( $newNode->nodeId);
                    $nodesProp[] = $newNode->nodeId;  
                }   
            }    
        
            if($nodesProp[0]>0){
                $idProduct = $this->db->fetchOne("select id_product from module_eshop_variants where model =?",$item['articl']);       
                $wProd = $this->db->quoteInto("id = ? ",$idProduct);
                $dataProd['prop'] = implode('|',$nodesProp);
             //   pr( $array);
             //   pr($dataProd);
                $this->db->update('content_Product',$dataProd,$wProd);
                $we = $this->db->quoteInto("id = ? ",$item['id']);
                $dae['insert_product'] = '1';
              //  pr($item['articl']);
                $this->db->update($this->_tableSepareteProd,$dae,$we);  
                //pr($item['articl']);
               // die;
            }
        }
    ...
    public function setSouvisejiciProduct()
    {  
        $data = $this->db->fetchAll("select * from ".$this->_tableSepareteProd);
        foreach($data as $item)
        {
            $dataSaveToDB['souvisejici'] = $item['properties'];
            $where = $this->db->quoteInto("id =?", $item['id_product']);
            $this->db->update("content_Product",$dataSaveToDB,$where);
        }  
    }
