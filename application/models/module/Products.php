...
{   
	...
	private $_filterNew = array(
			'znacka' => array('metric' => '','type'=>'checkBox','pos' => 1),			
			'color' => array('metric' => '','type'=>'checkBox','pos' => 2),
			'chlazeni' => array('metric' => '','type'=>'checkBox','pos' => 3),
			'type' => array('metric' => '','type'=>'checkBox','pos' => 4),
			'pocZonFilr' => array('metric' => '','type'=>'checkBox','pos' => 5),
		...
		private $_colors = array(
				...
				4 => '#E6E8FA', //Střibrná
				5 => '#50021B', //bordeux
				6 => '#E2725B', //Terra (hnědé
				7 => '#1E1E1E' //BlackSteel - (černá ocel)'
				);
	...
	public function setAdwordsXLS()
	{  
		$categories = array(76933,76932,76934,76936,76935);
		$data = array();  
		foreach($categories as $item)  
		{
			$retArray = $this->prepareDataAdwordXLS($item);
			$data = array_merge($data,$retArray);
		}
		...
		$list->setCellValue('A1', 'sestava = nadpis 1')  
			->setCellValue('B1', 'KW')  
			->setCellValue('C1', 'cílová URL')
			->setCellValue('D1', 'nadpis 1 (30)')      
			->setCellValue('E1', 'BID / KČ')      
		...
		$list->getColumnDimension("E")->setWidth(20);           
		$style = $list->getStyle("A1:E1");  
		$style->getFont()->setBold(true);   
		$inc = 2; 
		foreach ($data as $item=>$value) {   
				$list->setCellValue('A'.$inc, $value->sestava) 
					->setCellValue('B'.$inc, '"'.$item.'"')     
					->setCellValue('C'.$inc, "https://www.americkygril.cz".$value->url)  
					->setCellValue('D'.$inc, $value->nadpis)   
					->setCellValue('E'.$inc, $value->bid); 		  	   
				$inc++;    	 
			}
...
public function prepareDataAdwordXLS($category)
{
			//digestore
			$params['category'] = $category; 
			$data = $this->getProducts(false,false,false,false,$params);
			foreach($data as $item)
			{
					$object = new stdClass();  
					$object->url = $item['path'];
					$object->sestava = str_replace("","",$item['title']);
					$object->sestava = str_replace("","",$object->sestava );        
					$object->KW = $item['title'];
					$object->nadpis = $item['title'];
					$object->bid = $item['skladem'] == 1 ? '21' : '5';
					$array[$object->KW] = $object;
					$object->KW = str_replace("Weber ","",$item['title']);
					$object->KW = str_replace("®","",$object->KW );
					$array[$object->KW] = $object;
	...
	public function getHPSection( $sort = 'title', $sortType = 'Asc', $limitStart = 0, $limitCount = 5, $root, $count = false)    
    {
    	$select =  $this->db->select();
		$bind = array();
		$select->from(array( 'cm' => 'content_OverviewProducts'), array('n.id')); 		  
		...
		foreach ($result as $key => $value) {
			$nodes[] = $this->tree->getNodeById($value['id']);
		}
		return $nodes;  
	...
	{    
		...
		$objPHPExcel = new PHPExcel (); 
		$objPHPExcel->getProperties ()->setCreator ( "Specshop" )->setLastModifiedBy ( "Specshop" )->setTitle ( "Produkty" )->setSubject ( "Produkty" )->setDescription ( "Produkty" )->setKeywords ( "Produkty" )->setCategory ( "Produkty" );
			$styleArray = array(
				'font' => array(
						'bold' => true,
						'color' => array(
		...
		->setCellValue ( 'A1', 'Výpis produktů');
		$objPHPExcel->setActiveSheetIndex ( 0 )   
			->setCellValue ( 'A2', 'PageSeoTitle' )
		...
		$objPHPExcel->getActiveSheet()->getStyle('E'.$line.':K'.$line)->applyFromArray($styleArraya); 
		$allProducts = $this->getProducts('title','Asc',0,20000,$params);
		$line = 3;       
		foreach ($allProducts as $value) {
			$node = $this->tree->getNodeById($value['id']);
			$seoTitle = $node->getPropertyValue('pageTitle');			
			$pageDescription = $node->getPropertyValue('pageDescription');
			 $c = $node->getTheRightContent();            
			$objPHPExcel->setActiveSheetIndex ( 0 ) 
			->setCellValue ( 'A'.$line, $seoTitle)
			->setCellValue ( 'B'.$line, $pageDescription)
			->setCellValue ( 'C'.$line, $node->path);
			$line++;
		}     
		...
		e($objWriter->save ( "objednavkyRatan.xls" ));    
		e('dsfsd');        
	}	
	...
	public function setOrder($input)    
	{
		...
		$id = str_replace('reOrder', '', $input->id);
		$where = $this->db->quoteInto('id = ?', $id );
		$data['sold'] = $input->poradi;
	...
	public function renderNamesOption($view,$start = 1,$addZnacky = false)
	{
		$returnText = '';
 		$counter = $start;
		if(!$addZnacky){
			unset($this->_filterNew['znacka']);
		}
...
//    						e($value);
	...
	{
		$all = $this->getProducts(FALSE,false,0,9999999999999);
		foreach ($all as $value) {
			if(is_numeric(strpos(strtolower($value['title']), 'humibox'))){
				pr($value['title']);
				$data['znacka'] = 87;
...
public function getOptionsDB($view,$name, $params = array(), $count = false,$one = false)
	...
	private function setCustomOptions($view,$addZnacky = false){
	...
	public function showOptions($view,$params,$addZnacky=false)
	{
		$allOptions = $this->setCustomOptions($view,$addZnacky);
...
// 					pr($allOptions[0][$filterName]);
	...
	// projede zaškrtnuté kategorie a pokud není vloží, ty co nejsou v categories odmaže - přidá se NodeId pro eshop
	...
	private function showCompareProducts ($nodeId = false,$view,$showName = false)
	{
		...
		if($showName){
			...
			if($showName){
				...
				$html .= '<td>'.helper_FrontEnd::price($variant['price']).' Kč</a></td>';
			}
 ...
 public function renderBanerProduct()
	...
	public function initSearch( $sort = 'title', $sortType = 'Asc', $limitStart = 0, $limitCount = 5, $params,$view, $count = false)    
    {
		...
		if($count){    		 
    		...
    		$select->from(array( 'cm' => $this->_tableName), array($disc, 'cid' => 'cm.id', 'znacka', 'zboziProduct', 'cm.poradi as prodporadi', 'sold','n.id', 'n.title', 'n.path', 'n.parent', 'dateModif', 'dateCreate', 'html', 'files', 'parent', 'znacka', 'souvisejici',  'state', 'dphQuote', 'preText')); 		  
    	}  
        ...
        if($params['showAllV'])
        {
        	$select->join(
        			array('var' => $this->_tableNameVariants),
        			'var.id_product = cm.id',
        			array('id as variantId',  'EAN','inv_title', 'skladem', 'ext_id','model', 'poradi','purchase_price', 'obrazky', $price, 'price2', 'discount', 'realSold','skladem','heureka-hlid')
        	);       
        }          
        else{     
        $select->join(      
        		array('var' => $this->_tableNameVariants),     
        		'var.id_product = cm.id',    
        			array('id as variantId',  'EAN','inv_title', 'skladem', 'ext_id','model', 'poradi', 'purchase_price', 'obrazky', $price, 'price2', 'discount','heureka-hlid')
        );
        }
    ...
    public function getProducts( $sort = 'title', $sortType = 'Asc', $limitStart = 0, $limitCount = 5, $params = array(),$view = false)   
    {
    ...
    public function getProductsCout( $sort = 'title', $sortType = 'Asc', $limitStart = 0, $limitCount = 5, $params = array(),$view = false)   
    {   
