...
class module_Calculator{			
	...
	public function getTextProduct()
	{
		...
		$text = '<p class="big">
								'.helper_FrontEnd::price($this->_response['Deposit_min']/100).' Kč
								<span> dnes na splátky</span>  
								</p> 
								<p class="cofidis">  
	...
	// pomocne funkce 
        ...
        foreach( $fields as $field ) {
            if( preg_match('/([^:]+): (.+)/m', $field, $match) ) {
                                               ...
                                               $match[1] = preg_replace_callback('/(?<=^|[\x09\x20\x2D])./', 'stuffy_callback', strtolower(trim($match[1])));
