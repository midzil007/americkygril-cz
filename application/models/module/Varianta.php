...
class module_Varianta{ 
	...
	//	'obrazky' =>  array('cMap'=> 'Obrázky', 'type' => 'MultiFileSelect','options' => array('showSelectFile' => true, 'inputWidth' => '300', 'maxFiles' => 20, 'showUploadFile' => true, 'uploadFileDirectoryNodeId' => 3880 )),
		...
		'model' =>  array('cMap'=> 'Articl', 'type' => 'Text', 'options'=>false),  
		...
		'update_product' => array('cMap'=> 'Neatualizovat skladovost', 'type' => 'Checkbox', 'options'=>false),
		'color' =>  array('cMap'=> 'Barva', 'type' => 'Select', 'selection'=> array(
			0 => 'Není vybráno',
			1 => 'Bílá',
			2 => 'Černá',  
			3 => 'Nerez',
			...
			5 => 'Popelavě šedá',
			6 => 'Kouřová',
			7 => 'BlackSteel - (černá ocel)',
			8 => 'Žlutá',  
			9 => 'Modrá',
			10 => 'Červená', 
			11 => 'Nerez - folie imitace nerezu',
			12 => 'Béžová', 
			13 => 'Popelavě šedá',  
			14 => 'Šedo-modrá',    
			15 => 'Jarní zelená',
	...
	private $_tableNodeContents = 'NodesContents';
	...
	{  
		...
		$select->join ( array (
			...
			'p.id as id_product' ));  
		$select->join ( array (
				'con' => $this->_tableNodeContents 
				), 'p.id = con.c_id', array ());
		$select->join ( array (
			'nodes' => $this->_tableNodes 
			), 'con.n_id = nodes.id', array (
			'nodes.id as itemNodeId' ));
		$select->where(" ext_id =?",$ext_id);
		$variant = (object)$this->db->fetchRow($select); 
		return $variant;  
	}
	...
	if($sleva)
	{
		...
		return $this->db->fetchRow ( 'select id,EAN,skladem,poradi,obrazky,'.$price.',price2,discount,realSold,sold,deleted,
				model,id_product,alert 
				 from ' . $this->_tableName . ' where id=?', $id );
	}
